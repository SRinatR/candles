...
function getTranslatedStatus(status: Order['status'], t: (key: string) => string): string {
    switch (status) {
...
export default function OrderDetailPage({ params }: OrderDetailPageProps) {
  const t = useTranslations('accountOrderDetailPage');
  ...
  return (
    <div className="space-y-8">
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem><BreadcrumbLink href={`/${locale}/`}>{t('home')}</BreadcrumbLink></BreadcrumbItem>
          <BreadcrumbSeparator><Slash /></BreadcrumbSeparator>
          <BreadcrumbItem><BreadcrumbLink href={`/${locale}/account`}>{t('account')}</BreadcrumbLink></BreadcrumbItem>
          <BreadcrumbSeparator><Slash /></BreadcrumbSeparator>
           <BreadcrumbItem><BreadcrumbLink href={`/${locale}/account/orders`}>{t('orders')}</BreadcrumbLink></BreadcrumbItem>
          <BreadcrumbSeparator><Slash /></BreadcrumbSeparator>
          <BreadcrumbItem><BreadcrumbPage>{t('orderBreadcrumb').replace('{orderNumber}', order.orderNumber)}</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" asChild> 
            <Link href={`/${locale}/account/orders`}>
                <ArrowLeft className="mr-2 h-4 w-4" /> {t('backToOrdersButton')}
            </Link>
        </Button>
      </div>
      ...
      <Card className="shadow-lg">
        <CardHeader className="bg-muted/30 p-6">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
            <div>
              <CardTitle className="text-2xl">{t('orderNumTitle').replace('{orderNumber}', order.orderNumber)}</CardTitle>
              <CardDescription>{t('placedOn')} {new Date(order.date).toLocaleDateString()}</CardDescription>
            </div>
            <Badge variant={getStatusBadgeVariant(order.status)} className="text-sm px-3 py-1">{getTranslatedStatus(order.status, t)}</Badge>
          </div>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          ...
          <Separator />
          <div>
            <h3 className="text-lg font-semibold mb-3">{t('orderSummary')}</h3>
            <div className="space-y-1 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">{t('subtotal')}:</span><span>{order.totalAmount.toLocaleString('en-US')} UZS</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">{t('shipping')}:</span><span>{t('free')}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">{t('taxes')}:</span><span>{t('calculatedAtCheckout')}</span></div>
              <Separator className="my-2" />
              <div className="flex justify-between font-semibold text-base"><span>{t('total')}:</span><span>{order.totalAmount.toLocaleString('en-US')} UZS</span></div>
            </div>
          </div>
          <Separator />
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-lg font-semibold mb-3">{t('shippingAddress')}</h3>
            ...
            <div>
              <h3 className="text-lg font-semibold mb-3">{t('billingAddress')}</h3>
              <address className="not-italic text-sm text-muted-foreground space-y-0.5"><p>Jane Doe</p><p>123 Main St, Apt 4B</p><p>Anytown, CA 90210</p><p>United States</p></address>
            </div>
          </div>
        </CardContent>
        <CardFooter className="bg-muted/30 p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
            <p className="text-sm text-muted-foreground">{t('needHelp')} <Link href={`/${locale}${t('contactPagePath')}`} className="text-primary hover:underline">{t('contactSupport')}</Link></p>
            <div className="flex space-x-3">
                <Button variant="outline" size="sm"><FileText className="mr-2 h-4 w-4" /> {t('viewInvoiceButton')}</Button>
                <Button variant="outline" size="sm"><Truck className="mr-2 h-4 w-4" /> {t('trackPackageButton')}</Button>
            </div>
        </CardFooter>
      </Card>
    </div>
